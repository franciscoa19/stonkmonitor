...
17. **Dome cross-arb blocked by Cloudflare** (2026-07-02). Dome API now sits behind Cloudflare bot protection — plain requests get HTTP 403 `error code: 1010`; browser-UA and even `curl_cffi` Chrome TLS-impersonation requests hang until timeout (0 bytes). Not fixable client-side (needs Dome to whitelist the key/IP). Symptom was `Dome /v1/polymarket/markets error:` (empty = `asyncio.TimeoutError`) every scan cycle. Fix: circuit breaker in `feeds/dome.py` — after 5 consecutive failures, pause all Dome calls for 30 min and log one clear line, auto-retrying after cooldown. Cross-platform Kalshi↔Polymarket arb is degraded until Dome access is restored.
16. **Stale pending trades never expired across restarts** (fixed 2026-06-15). Per-trade expiry is an in-memory `asyncio.create_task(self._expire(...))` that sleeps 5 min then flips the row to `expired`. If the backend restarts before that fires, the task is lost and the DB row stays `pending` forever (observed: JD/DELL/XBI rows from 2026-04-21 still `pending` in June, polluting `/api/trade/queue` and the dashboard). Fix: `db.expire_stale_pending_trades()` runs once in the lifespan startup, flipping any `pending` row whose `expires_at < utcnow()` to `expired`. Startup log: `Startup sweep: expired N stale pending trade(s)`.
18. **No market-holiday calendar → stale-data ingestion on closed days** (fixed 2026-07-03). `current_session()` only knew time-of-day + `weekday >= 5`, so on a full holiday (e.g. Fri 2026-07-03, Independence Day observed) it returned `rth` and kept polling options-flow/darkpool every 15s. UW serves the last session's data on a closed market, so the feed ingested already-expired-option "sweeps" (e.g. a SPY call expiring 2026-07-02 surfacing 07-03) and burned budget. Fix: `MARKET_HOLIDAYS` / `MARKET_EARLY_CLOSES` tables + `is_market_closed_now()` in `feeds/uw_budget.py`; `current_session`/`market_subphase`/`is_auto_trade_window` now treat full holidays and post-13:00 early-closes as closed (→ `weekend` schedule, options/darkpool interval -1). Logs a warning if the running year isn't in the table (fail-open). **Update the tables annually.**
19. **Restart after downtime floods the feed with weeks-old filings as "live"** (fixed 2026-07-03). Two coupled defects: (a) `Signal.timestamp` always defaulted to `datetime.utcnow()`, so congress/insider signals were stamped with ingest time, not their filing date — the frontend showed a 3-week-old PTR as "now"; (b) no age gate, so on a restart after downtime the whole UW "recent" window re-scored as new (observed: backend down 2026-06-15→07-03 → 54 backlogged signals dumped at once, mostly June-17 filings). Fix: `score_congress`/`score_insider` parse `filed_at_date`/`filing_date`, stamp `Signal.timestamp` with it, and drop filings older than `CONGRESS_MAX_AGE_DAYS` (10) / `INSIDER_MAX_AGE_DAYS` (5) — gated on FILING date since a PTR filed today is fresh even if executed weeks ago. Frontend `SignalFeed` tile shows a "Jun 17 · 16d old" badge (amber ≥4d) from the signal's own timestamp, distinct from `_rx` receive-time. Verified live: 4 stale June-17 filings hit the raw table but 0 leaked past the gate into signals.
...
- ✅ **Earnings calendar + UW-optional mode (2026-09-09)** — two changes so the earnings IV/RV edge stands alone on **yfinance + Alpaca** and Unusual Whales can be cancelled. **(1) Earnings calendar:** `EarningsSetup.next_earnings_date` (from yfinance `get_earnings_dates()`/`.calendar`, free), stored in a new `iv_rv_evals.earnings_date` column; `iv_scanner_loop` now resolves each eval right after the real print (**earnings_date + 2d**) instead of the crude signal + 7d, falling back to +7d when no date is known. **(2) UW-optional:** `uw_enabled` flag (`UW_ENABLED`, default true) gates the UW stream task, the budget-monitor task, and the secondary IV-rank call; the UW API key is now optional so the app boots with it blank/cancelled. Set `UW_ENABLED=false` in `.env` — startup logs "Unusual Whales DISABLED … runs on yfinance + Alpaca only", no UW connection or error spam. All UW plumbing stays intact behind the flag. +`test_iv_rv_eval_stores_earnings_date` (18 tests). Verified: `next_earnings_date` populates (NVDA 2026-11-17, AAPL 2026-10-29); clean UW-disabled startup.
- ✅ **Curated earnings universe (2026-09-09)** — `signals/earnings_universe.py` (`EARNINGS_UNIVERSE` + `tickers(max_tier, sectors)`/`by_tier()` helpers): 78 tiered names chosen for the IV/RV scanner's gates (≥1.5M vol, weeklies for `ts_slope`, tight ATM straddle, real earnings-IV bump, $20–$1k). Tier 1 mega-cap (best short-straddle), 2 large-cap, 3 gap-prone (defined-risk only — TSLA/NFLX/META/COIN…). Lifespan startup seeds any missing universe names into the watchlist (declarative source; edit the file + restart to sync) → watchlist now 88 (curated 37 ∪ universe). Tier tags will drive sizing when Phase-2 execution is built.
- ✅ **IV/RV edge validator — Phase 1 (2026-09-09)** — validates the IV/RV "sell premium" edge on paper before building execution. `iv_rv_evals` table + `db.record_iv_eval/get_due_iv_evals/resolve_iv_eval/get_iv_eval_summary`. `iv_scanner_loop` logs each earnings_setup with its implied (straddle) move + entry price; the hourly `daily_equity_loop` resolves evals after a 7-day window by measuring the realized move vs implied (realized < implied ⇒ hypothetical short-straddle seller wins). Hit-rate + avg-edge surfaced in the daily report (`iv_rv`) and dashboard card. No execution yet — Phase 2 (short-straddle on Alpaca paper) only if the hit-rate proves out. 17 tests.
- ✅ **Strategy pivot: paused flow/pattern trades, curated watchlist (2026-09-09)** — the flow/pattern edge proved bad: `triple_confluence` went **0-for-5** (−$1,462, every trade −35% to −48%). Added `AUTO_TRADE_FLOW_ENABLED` (gates `evaluate_signal`/`evaluate_pattern`) and set it false in `.env` — pauses all sweep/golden_sweep/insider/congress/pattern auto-trades (engine infra kept). Turned the `$100B` market-cap auto-add off (`WATCHLIST_AUTO_ADD_MIN_MKTCAP=0` — it had sprawled to 120 names) and reset the watchlist to a curated 37 (Mag 7 + liquid mega-caps + high-IV movers + ETFs). Direction: make the IV/RV earnings scanner the primary signal — but the engine has no premium-selling execution yet, so next is to instrument/validate the IV/RV edge on paper (implied-vs-realized) before building short-straddle execution.
- ✅ **Watchlist auto-add for big/liquid names (2026-09-07)** — `_maybe_auto_watchlist()` in `process_uw_event` auto-adds a ticker to the IV/earnings watchlist when options flow appears on it and its `marketcap` ≥ `WATCHLIST_AUTO_ADD_MIN_MKTCAP` (0=off; set to 100e9 in `.env` for $100B+). Persisted + dedup'd; the weekly review prunes any that go quiet. Also manually added MU + SNDK (weekly review ADDs). Watchlist now 33 names.
- ✅ **Report guard vs. transient account-fetch failure (2026-09-04)** — a blip in `trader.get_account()` returns equity 0, which made `generate_daily_report` build + push + (would email) a bogus "$0 / −100%" report (seen once in the log). Now it bails early if `account.error` or equity is falsy, keeping the last good report and letting the scheduler retry next cycle. 16 tests.
- ✅ **Durable history + GitHub backup (Phase 0d, 2026-09-03)** — `export_history()` writes `backend/reports/history.jsonl` (equity curve, one row/day) + `trades.csv` (every closed, attributed trade); these + `latest.json` are **tracked in git** (only `reports/*.html` is ignored). `generate_daily_report` calls it, then `_git_push_eval_data()` commits + pushes just those files (staged individually, `pull --rebase`, best-effort) when `REPORT_GIT_PUSH=true` — so **GitHub is the off-machine durable backup** AND the bridge a cloud delivery routine reads from (cloud routines can't reach the local backend/localhost). Planned delivery: a cloud routine pulls git → republishes the dashboard artifact → emails digest (Gmail connector) + Google Drive DB snapshot (both need claude.ai connectors). 15 tests.
- ✅ **Daily/weekly report scheduler — Phase 0c backend core (2026-09-03)** — `report_scheduler_loop()` fires once per weekday at `REPORT_HOUR_ET` (default 8am ET) and runs the weekly watchlist review on Mondays; `generate_daily_report()` builds the report, persists it to `backend/reports/` (`daily_YYYY-MM-DD.html`, `latest.html`, `latest.json`), and Pushover-pings a one-line summary — fully autonomous, no Claude needed. Restart-safe (dedups on the day's report file). `build_watchlist_review()` proposes data-driven ADDs (un-watchlisted tickers with ≥8 signals/14d) and REMOVALs (watchlist names with 0 signals+trades/14d, guarded so freshly-added names aren't flagged). `POST /api/report/generate?weekly=` forces a run. Config: `REPORT_ENABLED`, `REPORT_HOUR_ET`. 14 tests. **Still needed for full delivery:** a scheduled *Claude* routine to republish the artifact + surface proposals for approval (backend can't republish a claude.ai artifact).
- ✅ **Eval-loop P&L fixes + test suite (2026-09-03)** — two reporting bugs fixed: (1) realized P&L is now booked for **server-side bracket exits** (which never call `record_exit`) via `db.reconcile_trades()` — pairs filled buy entries with sell exits per symbol, ×100 for OCC options (`_is_occ`), writes onto the entry row (keeps strategy attribution), run each perf-sync cycle; (2) the equity baseline no longer gets clobbered — `daily_equity.open_equity` holds the day's immutable open and the report's `start_equity` reads it. Added **`backend/tests/` (pytest, 12 tests, all mock-data / no network)** covering the P&L math, attribution join, immutable baseline, and end-to-end report metrics; `pytest.ini` (asyncio_mode=auto), `requirements-dev.txt`. Backfilled the live account: URI −$300 / DXCM −$95 (both triple_confluence), report now correctly shows −$585 (−1.17%), 0W/2L.
- ✅ **Fully autonomous paper mode + Telegram mute (2026-09-03)** — `AUTO_TRADE_AUTO_EXECUTE` (default false): when true, `_queue` calls `confirm_trade(id, msg_id=0)` immediately instead of waiting for a Telegram/UI confirm tap, so strategies run unattended (all pre-flight filters + risk caps + burst/daily limits still apply; paper only). `TELEGRAM_ALERTS_ENABLED` (default true) folds into `TelegramNotifier.enabled` — false mutes every send AND the getUpdates poll loop (killing the long-standing multi-instance conflict) while keeping all code intact. Both set in `.env` (local) for the new $50k autonomous setup. `DISCORD_ALERTS_ENABLED` (default true) works the same way — muted for this setup. Pushover left on.
...
- ✅ **Frontend watchdog + autostart (2026-07-02)** — `frontend/start_frontend.bat` restart loop (logs to `frontend/logs/frontend.log`) + `StonkMonitorFrontend.vbs` in the Startup folder, mirroring the backend. Frontend was previously unsupervised and kept dying on crash/reboot.
- ✅ **Dome circuit breaker (2026-07-02)** — `feeds/dome.py` pauses 30 min after 5 consecutive failures to stop 15s-timeout spam from the Cloudflare block (see Past Bugs #17).
- ✅ **Maintenance checklist (2026-07-02)** — `MAINTENANCE_CHECKLIST.md` at repo root: the required scan→update→review→approve→restart→secret-scan→push process for every change.
- ✅ **Dependency security pass (2026-06-15)** — patched 44 CVEs. Backend: `fastapi` 0.115→0.137.1, `uvicorn` 0.30.6→0.49.0, `aiohttp` 3.10.10→3.14.1 (24 CVEs), `python-dotenv` 1.0.1→1.2.2, `pydantic` 2.9.2→2.13.4, `pydantic-settings` 2.5.2→2.7.1, `starlette` 0.38→1.3.1; plus transitive floors (`urllib3`≥2.6.3, `requests`≥2.34, `idna`≥3.15, `protobuf`≥6.33.5, `curl-cffi`≥0.15). `pip-audit` now clean. Frontend: `next` 14.2.15→14.2.35 (resolves critical auth-bypass), `postcss`→^8.5.10. Remaining 2 npm advisories need breaking Next 16 — deferred (localhost-only single-user dashboard, DoS/cache-poisoning advisories require a hostile remote client). `requirements.txt`/`package.json` pins updated; backend + frontend build/run verified.
- ✅ **Startup pending-trade sweep** — `db.expire_stale_pending_trades()` in lifespan clears `pending` rows orphaned by restarts (see Past Bugs #16).
