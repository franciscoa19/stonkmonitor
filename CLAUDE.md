...
- ✅ **Autonomous paper-trading eval loop — Phase 0a (2026-09-03)** — foundation for the self-improving paper bot: strategy attribution (`pending_trades.strategy` captured from the triggering signal type / pattern name via `AutoTradeEngine._current_strategy`; `trade_performance` gains `strategy`/`entry_hour_et`/`hold_minutes`), a `daily_equity` snapshot table + hourly `daily_equity_loop`, and a `db._migrate()` step that ALTERs missing columns on existing DBs. Paper auto-trade enabled for the baseline (`.env` local only). Next: perf-sync attribution join, daily report artifact + scheduler, eval harness. Decisions/plan in the automation memory.
